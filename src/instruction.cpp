...
Instruction::Instruction(uint32_t rawInstruction) {
    ...
    if (opcode == 0x63) { 
        if (funct3 == 0x1) {// Bne 
        int32_t imm_12 = (rawInstruction >> 31) & 0x1;  // Bit 31 (sign bit)
        int32_t imm_11 = (rawInstruction >> 7) & 0x1;   // Bit 7
        int32_t imm_10_5 = (rawInstruction >> 25) & 0x3F; // Bits 30-25
        int32_t imm_4_1 = (rawInstruction >> 8) & 0xF;  // Bits 11-8

        // Construct signed immediate (sign-extend to 32-bit)
        imm = (imm_12 << 12) | (imm_11 << 11) | (imm_10_5 << 5) | (imm_4_1 << 1);

        // Sign-extension for negative offsets (if imm_12 is set)
