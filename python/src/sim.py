        ...
        try:
            with open(filename, 'r') as file:
                ...
                for line in lines:
                    ...
                    #     label = None
                    #     if ':' in tokens[0]:  # Check if there is a label
                    #         parts = tokens[0].split(':', 1)  # Split at the first colon
                    #         label = parts[0].strip()
                    #         self.data_labels[label] = data_address

                    #         # If there is content after ":", merge it back with tokens
                    #         if len(parts) > 1 and parts[1].strip():
                    #             tokens = [parts[1].strip()] + tokens[1:]
                    #         else:
                    #             tokens = tokens[1:]  # Remove the label token
                    ...
                    if in_data_section:
                        ...
                        # Check if the first part of the line contains a label (with or without space before the colon)
                        label_match = re.match(r'^(\w+)\s*:', line)
                        if label_match:
                            label = label_match.group(1)  # Extract label name
                            self.data_labels[label] = data_address
                            line = line[label_match.end():].strip()  # Remove label part and trim

                            # Re-tokenize after removing label
                            tokens = line.split()
